...
const ENGINE_API_URL = engineUrl(ENGINE_MODELS.floorImages);
...
export async function POST(req: NextRequest) {
  ...
  try {
    const { images, categories = [], prompt } = await req.json();
    ...
    const finalPrompt =
      typeof prompt === "string" && prompt.trim().length > 0
        ? prompt
        : buildFloorImagesPrompt(images.length, normalizedCategories);
    ...
    if (result) {
      if (recovered) {
        console.warn(
          `[floor-images] recovered a truncated response (finishReason=${finishReason ?? "unknown"})`,
        );
      }
