...
export async function PATCH(request: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  ...
  const { id } = await params; const body = (await request.json()) as Record<string, unknown>; const allowed = ["account_type", "status", "contract_name", "contract_start_date", "contract_end_date", "support_email"]; const update = Object.fromEntries(allowed.filter((key) => key in body).map((key) => [key, typeof body[key] === "string" ? String(body[key]).trim() || null : body[key]]));
